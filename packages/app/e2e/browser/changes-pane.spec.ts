...
test("selects, copies, and dismisses diff text without opening a review", async ({
  context,
  page,
}) => {
  ...
  await test.step("copy the selected characters with the keyboard", async () => {
    ...
    await expect.poll(readClipboard).toBe("CDEFGH");
    await expect(page.getByTestId("inline-review-editor")).toHaveCount(0);
  });
  await test.step("copy the selection and the whole line from the context menu", async () => {
    ...
    await expect.poll(readClipboard).toBe("ABCDEFGHIJ");
  });
